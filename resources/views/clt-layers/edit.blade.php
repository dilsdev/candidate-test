<x-app-layout>
    <div x-data="editLayer({{ $supplier->id }}, {{ $layup->id }}, {{ $layer->id }})" x-init="init()">
        <!-- Breadcrumb -->
        ...
        <div class="max-w-lg">
            <div class="card">
                <div class="card-header">
                    <h2 class="text-lg font-semibold text-gray-900">Edit Layer #{{ $layer->layer_order }}</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Update layer properties.</p>
                </div>
                <div class="card-body">
    ...
    <script>
        function editLayer(supplierId, layupId, layerId) {
            return {
                supplierId,
                layupId,
                layerId,
                ...
                async submit() {
                    ...
                    try {
                        await apiRequest(
                            `/api/suppliers/${this.supplierId}/layups/${this.layupId}/layers/${this.layerId}`, {
                                method: 'PUT',
                                body: JSON.stringify(this.form),
                            });
                        showToast('Layer updated successfully.');
                        window.location.href = `/suppliers/${this.supplierId}/layups/${this.layupId}`;
                    } catch (e) {
                        if (e.errors) this.errors = e.errors;
                        else showToast(e.message || 'Failed to update layer', 'error');
                    } finally {
