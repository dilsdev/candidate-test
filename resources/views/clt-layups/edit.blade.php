<x-app-layout>
    <div x-data="editLayup({{ $supplier->id }}, {{ $layup->id }})" x-init="init()">
        <!-- Breadcrumb -->
        <nav class="flex items-center text-sm text-gray-500 mb-6">
            ...
            <a href="{{ route('suppliers.layups.index', $supplier) }}"
                class="hover:text-gray-700 transition-colors">Layups</a>
            <svg class="w-4 h-4 mx-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
            <span class="text-gray-900 font-medium">Edit {{ $layup->name }}</span>
        </nav>

        <div class="max-w-lg">
            <div class="card">
                <div class="card-header">
                    <h2 class="text-lg font-semibold text-gray-900">Edit Layup</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Update layup configuration.</p>
                </div>
                <div class="card-body">
                    <div class="mb-6">
    ...
    <script>
        function editLayup(supplierId, layupId) {
            return {
                supplierId,
                layupId,
                form: {
                ...
                async submit() {
                    ...
                    try {
                        ...
                        else showToast(e.message || 'Failed to update layup', 'error');
                    } finally {
